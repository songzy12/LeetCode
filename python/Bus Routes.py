class Solution:
    def numBusesToDestination(self, routes, S, T):
        ...
        while queue:
            cur = queue.pop(0)
            # if the check is not here,
            # then we may miss S == T
            if cur == T: 
                return distance[cur]
            for route_index in invert[cur]:
                if route_index in visited_route:
                    continue
                visited_route.add(route_index)
                for stop in routes[route_index]:
                    if stop in visited_stop:
                        continue
                    # if the add to visited is not here,
                    # then it may be added twice to queue in the for loop
                    visited_stop.add(stop)
                    distance[stop] = distance[cur]+ 1
                    queue.append(stop)
        return -1
